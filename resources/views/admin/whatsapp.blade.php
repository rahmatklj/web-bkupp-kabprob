...
document.addEventListener('DOMContentLoaded', function() {
    if (typeof tinymce !== 'undefined') {
        tinymce.init({
            selector: '#wa_default_msg_editor',
            height: 200,
...
<div class="space-y-6 max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
        <form action="{{ route('admin.whatsapp.update') }}" method="POST" enctype="multipart/form-data" @submit="syncWATinyMCE()" class="space-y-5 text-xs">
            @csrf
            ...
            <div>
                ...
                <textarea id="wa_desc_editor" name="whatsapp_desc" rows="3" placeholder="Pengaduan & konsultasi cepat terhubung langsung ke WhatsApp resmi DKUPP..." class="w-full px-3.5 py-2.5 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:outline-none leading-relaxed text-slate-700">{{ $waDesc }}</textarea>
            </div>
