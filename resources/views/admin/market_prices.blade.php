...
@section('page_title', 'Kelola Link Website Harga Pasar (Simadu)')
...
<div class="space-y-6 max-w-4xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
        ...
        <a href="{{ $marketWebUrl  }}" target="_blank" rel="noopener noreferrer" 
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm p-6 sm:p-8 space-y-6">
        ...
        <form action="{{ route('admin.market-prices.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5 text-xs">
            ...
            <div class="p-4 bg-amber-50 rounded-2xl border border-amber-200 space-y-2">
                ...
                <input type="url" name="website_url" value="{{ old('website_url', $marketWebUrl ?? 'https://simadu.probolinggokab.go.id/') }}" required
            ...
            <!-- Upload File Logo & URL Logo Simadu (Tergabung di Menu Monitoring Harga Pasar) -->
            <div class="space-y-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                <label class="font-extrabold text-slate-800 block text-xs">
                    <i class="fas fa-image text-emerald-600 me-1"></i> Logo Website Simadu / Pemantauan Harga Pasar
                </label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 items-start">
                    ...
                    <div>
                        <label class="block font-bold text-slate-700 mb-1"><i class="fas fa-link text-slate-500 me-1"></i> Atau Masukkan URL Gambar Logo Simadu</label>
                ...
                @if(!empty($marketLogo))
                    <div class="mt-2 p-3 bg-white rounded-xl border border-slate-200 inline-flex items-center gap-3">
                        ...
                        <img src="{{ $marketLogo }}" alt="Logo Simadu" class="h-10 w-auto max-w-xs object-contain rounded border border-slate-200 bg-slate-50 p-1">
                    </div>
                @endif
            </div>
